...
-- 영엽부서인 사원의 사원명, 폰번호, 부서명, 휴가사용 이유 조회
...
SELECT * FROM DEPARTMENT D LEFT OUTER JOIN UNIT U
	ON D.UNIT_ID = U.UNIT_ID ORDER BY U.UNIT_NAME;

-- 2017년부터 2018년도까지 입사한 사원들의 사원명, 입사일, 연봉, 부서명 조회
-- 단, 퇴사한 사원들 제외 , 소속본부를 모두 조회
...
-- 학생들이 선택하지 않은 과목을 조회
SELECT * FROM SUBJECT SU LEFT OUTER JOIN STUDENT ST
    ON SU.SID = ST.SID
...
SELECT DISTINCT E.EMP_ID, E.EMP_NAME, E.HIRE_DATE, E.SALARY, D.DEPT_ID, D.DEPT_NAME
...
-- 홍길동 사원이 관리하는 사원들의 매니저사원아이디, 매니저사원명, 연봉, 매니저가 관리하는 사번, 매니저가 관리하는 사원명 조회
...
SELECT EMP_ID, EMP_NAME, MGR FROM EMP ORDER BY MGR;

/**************************************************
	SUB QUERY(서브쿼리) : SELECT ~ FROM 
...
SELECT VACATION_ID, EMP_ID, BEGIN_DATE, REASON -- 컬럼리스트는 컬럼명을 정확하게 작성하는 것을 권장, 개발시 
	FROM VACATION 
...
SELECT *
	FROM EMPLOYEE
    WHERE DEPT_ID IN (	-- 부서아이디가 2개 이므로 다중행 서브쿼리 형식은 IN 단일행 일때는 =
						SELECT DEPT_ID 
						FROM DEPARTMENT
						WHERE UNIT_ID = (SELECT UNIT_ID FROM UNIT WHERE UNIT_NAME = '제3본부')
    );
...
**/
-- 정보시스템 부서 사원들 모두 조회
SELECT ROW_NUMBER() OVER(ORDER BY EMP_ID) AS NO, EMP_ID AS ID, EMP_NAME AS NAME, HIRE_DATE
	FROM EMPLOYEE
	WHERE DEPT_ID = (SELECT DEPT_ID FROM DEPARTMENT WHERE DEPT_NAME = '정보시스템');
...
SELECT ROW_NUMBER() OVER(ORDER BY VCOUNT DESC) AS NO, 
	E.EMP_ID, E.EMP_NAME, E.HIRE_DATE, CONCAT(FORMAT(E.SALARY, 0), '만원') AS SALARY, V.VCOUNT '휴가사용일수'
	FROM EMPLOYEE E, 
    (SELECT EMP_ID, SUM(DURATION) VCOUNT FROM VACATION GROUP BY EMP_ID) V -- 인라인뷰의 별칭은 반드시 정의!!
...
SELECT ROW_NUMBER() OVER(ORDER BY E.EMP_ID) AS NO, 
	E.EMP_ID, E.EMP_NAME, E.DEPT_ID, (SELECT DEPT_NAME FROM DEPARTMENT WHERE DEPT_ID = 'HRD') AS DNAME, VCOUNT
...
-- 사원별 휴가사용 일수를 그룹핑하여,
-- 사원아이디, 사원명, 입사일, 연봉, 휴가결제횟수, 휴가전체사용일수를 조회
...
SELECT	ROW_NUMBER() OVER(ORDER BY COUNT DESC) AS NO,
		E.EMP_ID, E.EMP_NAME, E.HIRE_DATE, CONCAT(FORMAT(E.SALARY, 0), '만원') AS SALARY, 
...
-- 스킬라 서브쿼리 : MYSQL에서는 사용이 가능하나 권장하지 않음, ORACLE, DB2 등 데이터베이스는 사용불가
-- HRD 부서의 사원들의 사원아이디, 사원명, 부서아이디, 부서명 조회
SELECT EMP_ID, EMP_NAME, DEPT_ID, 
	(SELECT DEPT_NAME FROM DEPARTMENT WHERE DEPT_ID = 'HRD') AS DEPT_NAME,
	(SELECT UNIT_NAME FROM UNIT WHERE UNIT_ID = (SELECT UNIT_ID FROM DEPARTMENT WHERE DEPT_ID = 'HRD')) AS UNIT_NAME
	FROM EMPLOYEE
    WHERE DEPT_ID = 'HRD';
...
SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC)AS NO,
	   EMP_ID, EMP_NAME, DEPT_ID
       FROM EMPLOYEE;
       
SELECT NO, EMP_ID, EMP_NAME, DEPT_ID, HIRE_DATE
	FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC)AS NO,
			   EMP_ID, EMP_NAME, DEPT_ID, HIRE_DATE
			   FROM EMPLOYEE) T1 -- 인라인뷰 만들때 별칭은 필수
	WHERE NO <= 10;

-- 사원들의 급여 합계가 가장 작은 부서의 사원들을 조회
SELECT * 
	FROM EMPLOYEE 
...
-- 2013~2016 년도 사이에 입사한 사원과 
-- SYS부서의 사원들의 아이디, 사원명, 부서아이디, 폰번호, 연봉 조회\
-- UNION ALL(중복허용, 전체 데이터 모두 출력) : 22
-- UNION(중복허용 X, 전체 데이터 모두 출력) : 17
SELECT COUNT(*)
...
SELECT NO, CONCAT(YEAR, '년도') AS YEAR, DEPT_ID, CONCAT(FORMAT(SALARY, 0), '만원') AS SALARY FROM VIEW_SUM_SALARY;
...
AS
SELECT E.EMP_ID AS MGR_ID, E.EMP_NAME AS MGR_NAME, M.EMP_ID, M.EMP_NAME, M.DEPT_ID, D.DEPT_NAME
		FROM EMP E, EMP M, DEPARTMENT D
...
SELECT ROW_NUMBER() OVER(ORDER BY MGR_ID) AS NO, EMP_ID, EMP_NAME, DEPT_ID, DEPT_NAME FROM VIEW_EMP_MGR WHERE MGR_NAME = '홍길동'; -- '정주고', '오감자'

-- [전체 사원의 휴가일수 조회 : 휴가를 사용한 사원정보 + 사용하지 않은 사원]
-- 사원별 휴가결제횟수, 휴가전체사용일수를 그룹핑하여,
-- 사원아이디, 사원명, 입사일, 연봉, 휴가결제횟수, 휴가전체사용일수, 부서아이디, 부서명, 소속본부를 조회
-- 단, 휴가를 사용하지 않은 사원의 휴가결제횟수, 휴가전체사용일수는 0값으로 할당
